...
function validateEnvironment() {
  const required = ['CAPROVER_PASSWORD'];
...
program
  ...
  .option('-m, --machine <name>', 'CapRover machine name', 'caprover1133onubuntu2204-s-1vcpu-2gb-amd-sfo3-01')
...
program
  ...
  .action(async (appName, options) => {
    try {
      ...
      const tarFile = `${appName}-${timestamp}.tar.gz`;
      
      console.log(`🚀 Starting quick deployment workflow for: ${targetDir}\n`);
      
      // Step 1: Package target directory
      console.log(`📦 Step 1: Packaging ${targetDir}...`);
      await runCommand('tar', [
        '--exclude', tarFile,
        '--exclude', 'node_modules',
        '--exclude', 'capgen.js',
      ...
      // Step 2: Deploy
      console.log('🚢 Step 2: Deploying to CapRover...');
      await runCommand('caprover', [
        'deploy',
        '--caproverName', options.machine,
        '--tarFile', tarFile,
      ...
      // Step 3: Configure port if specified
      if (options.port && options.port !== '80') {
        console.log(`⚙️  Step 3: Setting container port to ${options.port}...`);
        await runCommand('caprover', [
          'api',
          '--caproverName', options.machine,
          '--path', '/user/apps/appDefinitions/update',
      ...
      // Step 4: Scale if specified
      if (options.scale && options.scale !== '1') {
        console.log(`📈 Step 4: Scaling to ${options.scale} replicas...`);
        await runCommand('caprover', [
          'api',
          '--caproverName', options.machine,
          '--path', '/user/apps/appDefinitions/update',
      ...
      console.log(`🌐 Your app should be available at: https://${appName}.bruvbot.com.br`);
...
program
  ...
  .option('-u, --url <url>', 'Custom URL to test (default: http://appName.bruvbot.com.br)')
  .option('-s, --https', 'Use HTTPS instead of HTTP', false)
  .action(async (appName, options) => {
    try {
      const protocol = options.https ? 'https' : 'http';
      const url = options.url || `${protocol}://${appName}.bruvbot.com.br/`;
      
      console.log(`🧪 Testing ${url}...`);
      await runCommand('curl', ['-f', '-s', '--max-time', '10', url]);
...
program
  ...
  .action(async (appName, endpoint, options) => {
    try {
      const protocol = options.https ? 'https' : 'http';
      const baseUrl = `${protocol}://${appName}.bruvbot.com.br`;
      ...
      const { spawn } = require('child_process');
      const curl = spawn('curl', ['-s', '--max-time', '10', fullUrl]);
  ...
  .action(async (appName, port) => {
    try {
      ...
      const curl = spawn('curl', [
        ...
        'https://captain.bruvbot.com.br/api/v2/user/apps/appDefinitions/update'
      ]);
...
program
  ...
  .action(async (appName) => {
    try {
      console.log(`🏓 Pinging http://${appName}.bruvbot.com.br/...`);
      
      const { spawn } = require('child_process');
      const curl = spawn('curl', ['-s', '-I', '--max-time', '5', `http://${appName}.bruvbot.com.br/`]);
