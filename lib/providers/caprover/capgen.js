...
program
  ...
  .action(async (appName, options) => {
    try {
      ...
      console.log(`🌐 Your app should be available at: https://${appName}.${process.env.CAPROVER_DOMAIN}`);
...
program
  ...
  .action(async (appName) => {
    try {
      ...
      const { spawn } = require('child_process');
      const curl = spawn('curl', ['-s', '-I', '--max-time', '5', `http://${appName}.${process.env.CAPROVER_DOMAIN}/`]);
